...
const tharuzz_footer = "> 𝙃𝙄𝙍𝙐 - 𝙓 - 𝙈𝘿 - 𝙑 1🤍💫";
const tharuzz_image = "https://files.catbox.moe/88ec05.jpg";
const tharuzz_channelJid = "120363418953677198@newsletter";
const tharuzz_newsletterName = "HIRU X MD 👨‍💻";
...
cmd(
    {
      ...
      const react = async (msgKey, emoji) => {
        ...
        try {
            ...
            let list = "*🤍 \`HIRU X MD CINESUB MOVIE SEARCH RESULTS\`*\n\n";
       mvDatas.forEach((m, i) => {
...
${tharuzz_footer}`
    ...
    const qualityHandler = async (tUpdate) => {
     ...
     const tIndex = parseInt(tText.trim()) - 1;
      ...
      const tharuzz_URLdl = await fetchJson(`https://manojapi.infinityapi.org/api/v1/cinesubz-download?url=${encodeURIComponent(downloadPageUrl)}&apiKey=${cineApiKey}`);
      
      const dl_Path = tharuzz_URLdl.results;
      const dlcine_Link1 = tharuzz_URLdl.results.pix1;
      const dlcine_Link2 = tharuzz_URLdl.results.pix2;
